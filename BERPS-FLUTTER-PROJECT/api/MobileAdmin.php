...
class MobileAdmin extends CI_Controller
{
    ...
    public function empDTR()
    {
        ...
        return mobile_json([
            ...
            'employees'              => $this->_staff_select_payload($employees, $settingsID),
    ...
    private function _staff_select_payload($rows, $settingsID)
    {
        $dtrCounts = $this->_dtr_record_counts($settingsID);
        $payload = [];
        foreach ((array) $rows as $row) {
            $userId = (int) ($row->user_id ?? 0);
            $username = (string) ($row->username ?? '');
            $userIdKey = $userId > 0 ? (string) $userId : '';
            $usernameCount = $username !== '' ? (int) ($dtrCounts[$username] ?? 0) : 0;
            $userIdCount = $userIdKey !== '' ? (int) ($dtrCounts[$userIdKey] ?? 0) : 0;
            $dtrKey = $usernameCount > 0
                ? $username
                : ($userIdCount > 0 ? $userIdKey : ($username !== '' ? $username : $userIdKey));
            ...
            $payload[] = [
                ...
                'dtr_key'          => $dtrKey,
                'dtr_record_count' => max($usernameCount, $userIdCount),
            ];
        }
        return $payload;
    }

    private function _dtr_record_counts($settingsID)
    {
        $rows = $this->db
            ->select('IDNumber, COUNT(*) AS record_count')
            ->from('dtr')
            ->where('settingsID', $settingsID)
            ->group_by('IDNumber')
            ->get()
        ...
        $counts = [];
        foreach ((array) $rows as $row) {
            $key = trim((string) ($row->IDNumber ?? ''));
            if ($key === '') {
                continue;
            }
            $counts[$key] = (int) ($row->record_count ?? 0);
        }
        return $counts;
    }
